...
export async function testTodoController() {
  ...
  try {
    ...
    assert(
      ...
      "初期状態でisLoadingはfalseであるべき"
    );
    assert(
      controller.operationStates instanceof Map,
      "operationStatesはMapであるべき"
    );
  ...
  try {
    ...
    results.push(
      displayTestResult("操作状態の設定テスト", false, error.message)
    );
  }
  ...
  try {
    ...
    await controller.addTodo("Test Todo", "high", "2025年02月13日", "work");
  ...
  try {
    ...
    assert(
      shownErrorMessage.includes("Todoの追加に失敗しました"),
      "適切なエラーメッセージが表示されるべき"
    );
    assert(
      shownErrorMessage.includes("HTTP error! status: 500"),
      "エラーステータスが含まれるべき"
    );
  ...
  try {
    ...
    results.push(
      displayTestResult("日付フォーマット変換テスト", false, error.message)
    );
  }
